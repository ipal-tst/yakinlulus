...
export default function EditMaterialPage({ params }: EditMaterialPageProps) {
    ...
    // Fetch existing material data by ID
    const { data: materialItem, isLoading: isLoadingMaterial } = useQuery({
        queryKey: ["admin-material-detail", materialId],
        queryFn: async () => {
            const list = await academicService.getMaterials();
            return list.find((m) => String(m.id) === String(materialId)) || null;
        },
        enabled: !!materialId,
    });
    ...
    const { data: dbSubjects = [] } = useQuery({
        queryKey: ["academic-master-subjects-edit-page"],
        queryFn: async () => {
            try {
    ...
    useEffect(() => {
        if (materialItem) {
            ...
            setSubjectName(materialItem.subject_name || availableSubjects[0] || "Penalaran Matematika");
            setCategory(materialItem.category || "TEORI");
            setReadingTime(materialItem.reading_time_minutes || 15);
            setStatus(materialItem.status || "PUBLISHED");
            setVideoUrl(materialItem.video_url || "");
            setPdfUrl(materialItem.pdf_url || "");
            setContent(materialItem.content || "");
        }
    }, [materialItem]);
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            await academicService.updateMaterial(materialId, {
                title: title.trim(),
        ...
        return (
            <div className="min-h-screen bg-background flex items-center justify-center p-6">
                <div className="flex items-center gap-2 text-muted-foreground font-medium text-sm">
                    <Loader2 className="h-5 w-5 animate-spin text-primary" /> Memuat data materi...
                </div>
            </div>
        );
    }
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground hidden sm:block">
                                ID: <span className="font-mono">{materialId}</span>
                            </p>
                        </div>
                    </div>
                </div>
