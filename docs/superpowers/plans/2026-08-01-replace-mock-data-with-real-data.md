...
- [ ] **Step 2: Add real health check hooks**
```typescript
...
- [ ] **Step 3: Update health services rendering**
```typescript
...
### Task 2: Fix Master Data Levels (`admin/master-data/levels/page.tsx`) - Remove INITIAL_LEVELS Fallback
...
- [ ] **Step 1: Remove INITIAL_LEVELS constant**
```typescript
// DELETE lines 36-41:
const INITIAL_LEVELS: LevelItem[] = [...]
```

- [ ] **Step 2: Remove fallback state initialization**
```typescript
// REPLACE line 73:
...
- [ ] **Step 3: Remove mock ID check in delete**
```typescript
...
- [ ] **Step 4: Ensure fetchLevels uses only real API**
```typescript
...
- [ ] **Step 5: Add proper loading state**
```typescript
...
### Task 3: Fix Master Data Subjects (`admin/master-data/subjects/page.tsx`) - Remove INITIAL_SUBJECTS Fallback
...
- [ ] **Step 1: Remove INITIAL_SUBJECTS constant**
```typescript
// DELETE lines 35-43:
...
### Task 4: Fix Admin Dashboard - Add Real Health/Logs API Endpoints
...
- [ ] **Step 1: Create admin handler with health/logs endpoints**
```go
...
- [ ] **Step 4: Update admin dashboard to use real hooks**
...
### Task 5: Verify All Other Admin Pages Use Real Data (Audit Pass)
...
- [ ] **Step 1: Audit each page for mock constants**
```bash
# Check for INITIAL_* constants
grep -r "INITIAL_" frontend/app/(portal)/admin/
```

- [ ] **Step 2: Verify these pages use real hooks (already done):**
- [ ] `admin/analytics/page.tsx` - uses `useAnalyticsOverview`, `useAnalytics` ✓
- [ ] `admin/cbt/page.tsx` - uses `useExams`, `useCreateExam`, etc. ✓
- [ ] `admin/materials/page.tsx` - uses `useMaterials`, `useSubjects`, etc. ✓
- [ ] `admin/question-bank/page.tsx` - uses `useQuestions`, `useCreateQuestion`, etc. ✓
- [ ] `admin/users/page.tsx` - uses `useUsers`, `apiFetch` ✓
- [ ] `admin/schools/page.tsx` - uses `useSchools`, `useCreateSchool` ✓
- [ ] `admin/subscriptions/page.tsx` - uses `useSubscriptionStats`, `useSubscriptionPlans` ✓
- [ ] `admin/audit-logs/page.tsx` - uses `useAuditStats`, `useAuditLogs` ✓
- [ ] `admin/ai-tutor/page.tsx` - uses `useAIStats`, `useAIConversations` ✓
- [ ] `admin/content/page.tsx` - uses `useContents`, `useMedia` ✓
- [ ] `admin/settings/page.tsx` - uses `apiClient.ai.getConfig` ✓
- [ ] `admin/master-data/page.tsx` - uses `useLevels`, `useSubjects` ✓
- [ ] `admin/master-data/chapters/page.tsx` - uses `useChapters` (mutation fixed) ✓
- [ ] `admin/master-data/curriculums/page.tsx` - check for mock
- [ ] `admin/master-data/programs/page.tsx` - check for mock
- [ ] `admin/master-data/topics/page.tsx` - check for mock

- [ ] **Step 3: Fix any remaining pages with mock data**
- Add proper hooks where missing

---

### Task 6: Create Missing Admin Health/Logs API
...
- [ ] **Step 4: Add to queryKeys and hooks**
```typescript
// lib/query-keys.ts
...
- [ ] **Step 1: Remove all INITIAL_* constants**
```bash
